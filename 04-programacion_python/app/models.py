...
def get_database_path():
    # 'os.path.dirname(__file__)' obtiene la ruta del directorio actual donde se encuentra el archivo models.py
    current_dir = os.path.dirname(__file__)
    # Combina la ruta del directorio actual con el nombre del archivo
    return os.path.join(current_dir, 'database.json')


def load_database():
    with open(get_database_path(), 'r') as f:
...
def update_database(new: dict):
    with open(get_database_path(), 'w') as database:
...
class Model:
    ...
    def read(self, columns: List[str] = []):
        return {
            colname: self._columns[colname]._type(getattr(self, colname)) if self._columns.get(colname) else getattr(
                self, colname) for colname in (columns or list(self._columns.keys())) + ['id']}
